...
par(mfrow=c(2,2))
plot(strptime(paste(as.character(sdf$Date), as.character(sdf$Time), sep = ""), format = "%Y-%m-%d %H:%M"), as.numeric(levels(sdf$Global_active_power)[sdf$Global_active_power]), ylab = "Global Active Power", type = "l",xlab = "")
plot(strptime(paste(as.character(sdf$Date), as.character(sdf$Time), sep = ""), format = "%Y-%m-%d %H:%M"), as.numeric(levels(sdf$Voltage)[sdf$Voltage]), ylab = "Voltage", type = "l",xlab = "datetime")
...
plot(strptime(paste(as.character(sdf$Date), as.character(sdf$Time), sep = ""), format = "%Y-%m-%d %H:%M"), as.numeric(levels(sdf$Global_reactive_power)[sdf$Global_reactive_power]), ylab = "Global_reactive_power", type = "l",xlab = "datetime")
dev.off()
